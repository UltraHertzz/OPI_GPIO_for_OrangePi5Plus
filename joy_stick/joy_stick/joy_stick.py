...
class Joystick:
    ...
    def __init__(self,refreshRate = 30):
        ...
        if not found:
            ...
            #self.close() modify_LSX
            #raise IOError('Unable to detect Xbox controller/receiver - Run python as sudo') modify_LSX
            pass  # modify_LSX
...
class ControlPublisher(Node):
    ...
    def __init__(self, node_name: str, *, context: Context = None, cli_args: List[str] = None, namespace: str = None, use_global_arguments: bool = True, enable_rosout: bool = True, start_parameter_services: bool = True, parameter_overrides: List[Parameter] = None, allow_undeclared_parameters: bool = False, automatically_declare_parameters_from_overrides: bool = False) -> None:
        ...
        self.publisher = self.create_publisher(Float32MultiArray, '/js', 10)
        timer_period = 0.02
    ...
    def timer_callback(self):
       ...
       self.get_logger.info('Publishing: %d %d',msg.data)


def main():
    print('Publish Joystick Control Value')
    pub = ControlPublisher("joy_stick")
    rclpy.spin(pub)
    pub.destroy_node()
    rclpy.shutdown
